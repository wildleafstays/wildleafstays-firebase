...
async function dropProductChecks(db: Kysely<unknown>, tableName: string): Promise<void> {
  await sql.raw(`
    do $$
    declare
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`
    alter table reservations
      ...
      add column room_mix_inventory_hold_id uuid
  `.execute(db);

  await dropProductChecks(db, "reservations");

  await sql`
    alter table reservations
      add constraint reservations_product_type_allowed_check
        check (product_type in ('ROOM_CATEGORY', 'FULL_PROPERTY', 'ROOM_MIX')),
      add constraint reservations_product_identity_check
        check (
          (
            product_type = 'ROOM_CATEGORY'
            and room_category_id is not null
            and quote_id is not null
            and quote_inventory_hold_id is not null
            and room_mix_quote_id is null
            and room_mix_inventory_hold_id is null
          )
          or
          (
            product_type = 'FULL_PROPERTY'
          ...
          (
            ...
            and room_mix_inventory_hold_id is not null
          )
        ),
      add constraint reservations_room_mix_quote_fk
        foreign key (room_mix_quote_id, property_id, organization_id)
          references room_mix_quotes(id, property_id, organization_id) on delete restrict,
      add constraint reservations_room_mix_hold_fk
        foreign key (
          room_mix_inventory_hold_id,
          room_mix_quote_id,
          inventory_hold_id,
          property_id,
          organization_id
        )
          references room_mix_inventory_holds(
            id,
            room_mix_quote_id,
            inventory_hold_id,
            property_id,
      ...
      add constraint reservations_room_mix_link_key
        unique (id, room_mix_quote_id, property_id, organization_id)
  `.execute(db);

  await sql`
    create unique index reservations_room_mix_quote_unique_idx
      on reservations (room_mix_quote_id)
      where room_mix_quote_id is not null
  `.execute(db);

  await sql`
    create unique index reservations_room_mix_hold_unique_idx
      on reservations (room_mix_inventory_hold_id)
      where room_mix_inventory_hold_id is not null
  `.execute(db);

  await sql`
    alter table reservation_financial_snapshots
      alter column quote_id drop not null,
      alter column rate_plan_id drop not null,
      alter column rate_plan_code drop not null,
      alter column rate_plan_name drop not null,
      alter column meal_plan_code drop not null,
      alter column rate_product_id drop not null,
      alter column rate_product_version drop not null
  `.execute(db);

  await dropProductChecks(db, "reservation_financial_snapshots");

  await sql`
    alter table reservation_financial_snapshots
      add constraint reservation_financial_product_type_allowed_check
        check (product_type in ('ROOM_CATEGORY', 'FULL_PROPERTY', 'ROOM_MIX')),
      add constraint reservation_financial_product_identity_check
        check (
          (
            product_type = 'ROOM_CATEGORY'
            and quote_id is not null
            ...
            and rate_plan_name is not null
            and meal_plan_code is not null
            and rate_product_id is not null
            and rate_product_version is not null
            and room_category_id is not null
          )
          or
          (
          ...
          (
            ...
            and rate_plan_name is null
            and meal_plan_code is null
            and rate_product_id is null
            and rate_product_version is null
            and room_category_id is null
            and quantity >= 2
          )
        ),
      add constraint reservation_financial_reservation_fk
  ...
  await sql`
    create table reservation_room_mix_items (
      id uuid primary key default gen_random_uuid(),
      reservation_id uuid not null,
      room_mix_quote_id uuid not null,
      room_mix_quote_item_id uuid not null,
      ...
      foreign key (reservation_id, room_mix_quote_id, property_id, organization_id)
        references reservations(id, room_mix_quote_id, property_id, organization_id)
          on delete restrict,
      foreign key (
        room_mix_quote_item_id,
        room_mix_quote_id,
        quote_id,
        property_id,
        organization_id
      )
        references room_mix_quote_items(
          id,
          room_mix_quote_id,
          quote_id,
          property_id,
  ...
  await sql`
    create trigger reservation_room_mix_items_no_mutation
      before update or delete on reservation_room_mix_items
  ...
  await sql`
    ...
    begin
      ...
      if (
        ...
        new.room_mix_inventory_hold_id,
        ...
        old.room_mix_inventory_hold_id,
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`
    ...
    begin
      if exists (select 1 from reservations where product_type = 'ROOM_MIX') then
        raise exception 'cannot roll back room-mix migration while ROOM_MIX reservations exist';
      end if;
    end
    $$
  `.execute(db);
  ...
  await sql`
    drop trigger if exists reservation_room_mix_items_no_mutation
      on reservation_room_mix_items
  `.execute(db);
  await sql`drop table if exists reservation_room_mix_items`.execute(db);

  await sql`
    alter table reservation_financial_snapshots
      drop constraint if exists reservation_financial_reservation_fk,
      drop constraint if exists reservation_financial_product_identity_check,
      drop constraint if exists reservation_financial_product_type_allowed_check
  `.execute(db);
  ...
  await sql`
    alter table reservation_financial_snapshots
      ...
      alter column rate_plan_code set not null,
      alter column rate_plan_name set not null,
      alter column meal_plan_code set not null,
      alter column rate_product_id set not null,
      alter column rate_product_version set not null,
      add constraint reservation_financial_snapshots_product_type_check
        check (product_type in ('ROOM_CATEGORY', 'FULL_PROPERTY')),
      add constraint reservation_financial_snapshots_product_identity_check
        check (
  ...
  await sql`drop index if exists reservations_room_mix_hold_unique_idx`.execute(db);
  await sql`drop index if exists reservations_room_mix_quote_unique_idx`.execute(db);

  await sql`
    alter table reservations
      drop constraint if exists reservations_room_mix_link_key,
      drop constraint if exists reservations_room_mix_hold_fk,
      drop constraint if exists reservations_room_mix_quote_fk,
      drop constraint if exists reservations_product_identity_check,
      drop constraint if exists reservations_product_type_allowed_check
  `.execute(db);

  await sql`
    alter table reservations
      drop column if exists room_mix_inventory_hold_id,
